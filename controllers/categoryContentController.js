...
const getAllCategoryContentData = async (req, res) => {
    ...
    let contentData;
    try{
        contentData = await CategoryContent.find(); // Retrieve data from the database
...
const addCategoryContentUnderSingleCategory = async (req, res) => {
    try {
        ...
        const categoryExists = await PortfolioCategory.findById(req.params.categoryId); //get the category
        if (!categoryExists) {
            return res.status(404).json({ message: 'Category not found' });
        }

        const contentData = await CategoryContent.create(req.body); //adds content data to CategoryContent Document
        if (!contentData) {
...
const updateCategoryContentUnderSingleCategory = async (req, res) => {
    try {
      //check if category exists!
      const categoryExists = await PortfolioCategory.findById(req.params.categoryId);
      if (!categoryExists) {
        return res.status(404).json({ message: 'Category not found' });
      }
      ...
      const updatedContent = await CategoryContent.findByIdAndUpdate(
...
const getContentOfIndividualCategory = async (req, res) =>{
    try{
        const categoryContent = await PortfolioCategory.findById(req.params.categoryId).populate("contents");
