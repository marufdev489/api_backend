...
const getProductData = async (req, res) => {
    ...
    let productData;
    try{
        productData = await Product.find(); // Retrieve data from the database
...
const updateProductData = async (req, res) => {
    let data = req.body;
    try {
      const productDataExists = await Product.findById(req.params.id);
      if (!productDataExists) {
        return res.status(404).json({ message: 'Product entry not found' });
      }
      const productData = await Product.findByIdAndUpdate(req.params.id, data);
      if (!productData) {
...
module.exports = { getProductData, addProductData, updateProductData, deleteProductData };